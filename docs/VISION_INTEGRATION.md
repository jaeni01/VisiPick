# 비전 이식 노트 (염재니 비전 → 팀원(백엔드) 몸통, 통합방향 A)

> 인프라(api_server, db, agv_mqtt, serial_ctrl, robot, mqtt 토픽, camera_top/side,
> recipe_mgr, tray_mgr, state_machine 골격)는 **팀원(백엔드) 것 그대로**. 비전 경계만 교체.
...
4. **is_duplicate 부호 + 부품명 언어가 둘 다 반대.**
   Decision: `is_duplicate(part)=True → 중복`. 팀원(백엔드) `needs(part)=True → 아직 필요`(반대).
...
```
`dummy_mode=false` 면 `models/best.pt` 로드 후 실제 추론. 카메라가 없으면 팀원(백엔드)
