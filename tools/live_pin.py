...
from src.vision.camera_util import open_camera  # noqa: E402
from src.vision.pin_inspector import _PinDetector  # noqa: E402


def _frame_source(source: str):
    """프레임 제너레이터. frame 을 yield."""
    if source.isdigit():                      # 실 카메라: config.cameras.side 설정 적용(DSHOW+노출)
        cam = config["cameras"]["side"]
        cap = open_camera(cam, index=int(source))
